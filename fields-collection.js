...
// 03/12/2015 - This module does not look all that important.
//  Functionality could be folded into Data_Object.
//  .get and .set within Data_Object handle a lot of what happens with fields.

// For the moment, will change .get so that when the value has not been assigned, it gets an empty Data_Object.
//  So the Data_Object will have an unassigned value.
//   Then we can listen to that Data_Object for changes.

// Definitely needs overhauling.
//  The field definitions are not checked properly.
//  May give some types in the definition, such as Array.
//   While these are constructor functions, they need to be treated as if they are indicating an Array type is to be used.
//   So need the logic for handling sig [s,s,f]
	...
	'set'() {
		...
		if (sig == '[a]') {
			...
			if (ioa_sig == '[s,s]') {
				...
				// Is it efficient to pass around those constructor functions?

				//  It's a nice trick at one stage, but possibly a different internal representation would be better.
				//   Perhaps get rid of the constructor function where it's not necessary, and give a string description of the field's type.
				var field_arr;

				if (field_obj === Array) {
					field_str_def = 'array'
					field_arr = [field_name, field_str_def];
				} else if (field_obj === String) {
					field_str_def = 'string'
					field_arr = [field_name, field_str_def];
				} else if (field_obj === Number) {
					field_str_def = 'number'
					field_arr = [field_name, field_str_def];
				} else {

					field_arr = [field_name, field_str_def, field_obj];
